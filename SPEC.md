...
## Addendum 7/7: the round-2 multi-column claim was too broad

Corpus diagnostics round 2 above states "true multi-column never appears in any fixture's text layer." That generalization does not survive the 7/7 hard.pdf smoke test on the deployed app. What the 7/4 diagnostic actually established was narrower: easy.pdf's suspected column page (p3) is single-column in the text layer, and wide in-line gaps on measured pages were left/right aligned pairs. hard.pdf pages 15 to 18 are real two-column resume layouts whose sidebar content interleaves into body prose under y-band line assembly, and easy.pdf p7 carries an officer sidebar that interleaves the registration blocks (the golden set's scrambled-registration case exists because of it). The scoped measurement was right; the recorded conclusion overreached from "the pages I checked" to "any fixture."

The decision the claim supported is unchanged: the column engine stays cut for v1 (the pass-bar fixture's editing path does not need one, and the class definition plus the scan-is-rare steer both predate this correction), and a per-page column model is the first item on the v2 list. Full smoke-test findings and the day's persona-run receipts live in context/decisions.md 7/7.

## Addendum 7/8 (after midnight): the column model shipped, superseding the addendum above's "stays cut"

The 7/7 addendum said the column engine stays cut for v1. That held until every required artifact was done, verified, and pushed; the model was then built the same night on an isolated branch, measurement-first (channel geometry measured across both layout fixtures before code), and merged after passing a re-baselined golden set behind four declared-before-run eval baselines. Parser v6 ships: channel detection with measured thresholds, column-major reading order, rotated-decoration exclusion, and font-run subheading detection scoped to prose-dominated column streams. The re-baselining cost (one case retired because its defect no longer exists, four case coordinates or expectations re-authored) is reported in the README's eval section as the measured price of changing load-bearing code. Still cut, now with sharper names: infographic row-regions (the capacity chart misattributes roles by one), region-scoped channels (the mixed-region page keeps a split-sentence limit), and the font signal outside column streams (document-wide it shatters cover typography; measured and reverted). The log stages, the spec stays gospel by being corrected: same rule as every addendum before this one.

## Addendum 7/8 (afternoon): two of the prepared v2 answers shipped before submission

The demo prep section above stages a lightweight review pass (their consistency and name checks) as v2 material. It shipped, and so did tracked changes in the DOCX export: both were built on the morning of 7/8 on branches and merged behind a verification gate, the same rule the addendum above set. Details live in the README's roadmap graduations. The log stages, the spec stays gospel by being corrected.
