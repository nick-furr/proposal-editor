...
**Block-scoped edits can strand collateral facts, and a post-edit consistency pass now checks for exactly that.** Swap the client city in one block and a distance claim elsewhere in the document may quietly stop being true; clean debris out of a block and a name that existed nowhere else leaves the document entirely. After every apply, a deterministic scan diffs the edit for removed entities and searches every other block for them (the eval's name-fidelity primitive pointed at the live document), flags names that left the document entirely, and, only when it finds lexical hits, sends the hit blocks to one model call that judges which mentions actually went stale and drafts a follow-up instruction. Accepting a follow-up runs the normal loop: one reviewed diff, same audit trail. The limits are as designed and worth stating: recall is lexical, so a stale claim that shares no words with a removed entity is not caught; the judge sees only flagged blocks; and staleness inside the edited block itself is the reviewed diff's job, deliberately, because the human already has those words in front of them in red and green. If the judge call fails, the card degrades to the raw text matches rather than blocking the apply. The card survives unrelated applies (a tone edit between follow-ups must not destroy pending findings) but never goes stale doing it: every edit or undo that moves text revalidates the card's tracked entities against the live document, rows whose mention was fixed drop, a card with nothing left clears itself, and changed flagged text sends the judge back out because a verdict describes exact words. Undoing the apply a card reports on restores the card it replaced, revalidated, so findings that undo did not resolve stay visible. These lifecycle rules came out of hands-on verification on the deployed app and a pre-submission review pass, not the first design.
...
**DOCX export, with applied edits as real Word tracked changes.** Consultants finish in Word, per the team's own words, and they review in tracked changes. The export builds a Word document from the parsed structure, headings mapped to Word heading styles, and every edited block carries its net change as genuine Word revisions: deletions struck, insertions marked, author "Proposal Editor". Client-side, lazy-loaded, verified by opening the output in Word itself and enumerating the revision objects. Tracked rather than clean because the recipient of an exported proposal is usually not the person who edited it: Accept All recovers a clean document in two clicks, and the reverse is impossible. One piece stays unbuilt: the instruction as a revision comment on each change. The tracked diff is whole-block net change, so per-instruction comments need per-edit diff segments first.
...
**The post-edit consistency pass.** Built submission morning on a branch, after the core was verified and frozen, because the failure mode it addresses was already documented above and the receipts (the stranded-distance case, the officer-name removal) were already in the eval's vocabulary. The design is a hybrid chosen on cost measurement: a full-document model review would cost 5 to 15 times the edit itself on every apply and its latency grows with the document, while the deterministic scan is free and fires the single model call only when entities actually moved, so the common tone or length edit costs nothing and a name swap costs about one extra edit. The scan is deliberately the same primitive the eval's name-fidelity metric uses, pointed at the live document. Building it also surfaced and fixed a real coherence gap: section headings rendered from parse-time text, so a heading edit (newly reachable through the pass's follow-ups) displayed stale on screen while exporting correctly to Word. Found in the browser during verification, fixed the same hour.
...
In priority order, each continuing something the current build already measures or feels. This list is real: three items have graduated off it mid-take-home (the column model, the post-edit consistency pass, and tracked-changes export), each built on a branch, verified, and merged.
...
2. **Region-scoped channels and infographic row-regions.** The two named parser boundaries left after the column work: the page that mixes full-width prose with a three-column table region, and the capacity chart whose role labels shift by one person. The second is the sharper problem, because it misattributes rather than garbles.
3. **Grounding guards.** Entity-consistency between block and retrieved references, a retrieval relevance threshold, the mis-grounding case added to the golden set as a known-failing regression target, and a hostile-document case (instructions embedded in block text) to red-team the data-not-instructions separation.
4. **Provenance chips on grounded sentences.** A marker on any knowledge-base-grounded addition naming which past proposal the fact came from, click-through to the source excerpt. The retrieval already knows the source document; the user should too.
